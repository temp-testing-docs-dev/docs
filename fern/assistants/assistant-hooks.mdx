...
Assistant hooks allow you to configure actions that will be performed when specific events occur during a call. Currently, hooks support the following events:

- `call.ending` (triggered when a call is ending)
...
- `assistant.speech.started` (when the assistant starts speaking)
- `customer.speech.started` (when the customer starts speaking)

## Usage

Hooks are defined in the `hooks` array of an assistant. Each hook consists of:

- `on`: The event that triggers the hook (supports all events mentioned above)
- `do`: The actions to perform when the hook triggers (supports `transfer`, `function`, `say`, and `hangup`)
...
## Example: Handling Call Start

This example shows how to use the `call.started` event to perform actions when a call begins:
  ...
  "hooks": [{
    ...
    "do": [{
      ...
      "function": {
        ...
        "description": "Logs the start time of the call"
      },
      "async": true,
      "server": {
        "url": "https://your-server.com/api"
...
## Example: Using the Hangup Action

This example demonstrates how to use the `hangup` action to end a call under certain conditions:
...
curl -X PATCH "https://api.vapi.ai/assistant/<id>" \
  ...
  "hooks": [{
    ...
    "do": [
      {
        "type": "say",
        "exact": "I'm sorry, but our call time has reached its limit. Thank you for your time."
      },
      {
        "type": "hangup"
      }
...
- Logging call start and end times
- Performing actions based on call duration or other metrics
- Customizing behavior at different stages of the call

By leveraging these hooks, you can create more robust and responsive voice AI applications that can handle a wide range of scenarios and provide a better user experience.
