...
- `do`: The actions to perform when the hook triggers (supports `transfer`, `function`, `say`, and `log`)
...
## Example: Logging Call Start

This example shows how to log when a call starts using the new `call.started` event:
...
curl -X PATCH "https://api.vapi.ai/assistant/<id>" \
  ...
  "hooks": [{
    ...
    "do": [{
      "type": "log",
      "message": "Call started at {{currentDateTime}}"
    }]
  }]
}'
```

## Example: Handling Speech Start Events

You can use the new `assistant.speech.started` and `customer.speech.started` events to perform actions when either party begins speaking:
  ...
  "hooks": [
    {
      ...
      "do": [{
        "type": "log",
        "message": "Assistant started speaking at {{currentDateTime}}"
      }]
    },
    {
      "on": "customer.speech.started",
      "do": [{
        "type": "log",
        "message": "Customer started speaking at {{currentDateTime}}"
      }]
    }
  ]
}'
...
- Logging important call events for analysis and monitoring
- Triggering external actions based on specific call events
