...
You can control when call analysis runs based on the number of messages exchanged during the call using the `minMessagesThreshold` setting. This helps avoid running analysis on very short calls that may not contain meaningful content.

### Configuration

Configure the minimum message threshold in your assistant's `analysisPlan`:
...
- **Default**: If not specified, analysis runs on all calls regardless of message count
- **When set**: Analysis only runs if the call contains at least the specified number of messages
- **Message counting**: Includes all messages in the conversation (user, assistant, system, tool calls, etc.)

### Use cases

- **Skip short calls**: Avoid analyzing calls that end immediately due to wrong numbers, hang-ups, or technical issues
- **Focus on meaningful interactions**: Only analyze calls where substantial conversation occurred
- **Cost optimization**: Reduce analysis costs by filtering out very brief interactions

### Example configurations

```json
// Only analyze calls with at least 3 messages
{
  "analysisPlan": {
    "minMessagesThreshold": 3,
    "summaryPrompt": "Summarize this customer interaction"
  }
}
```

```json
// Only analyze substantial conversations (10+ messages)
{
  "analysisPlan": {
