...
<Warning>
Note: When creating the SIP trunk, it is crucial to provide all the signaling IP addresses. Failure to do so will prevent proper whitelisting, which may result in encountering unauthorized 401 errors for inbound calls. Make sure to include all relevant IP addresses to ensure smooth inbound call handling.
</Warning>
