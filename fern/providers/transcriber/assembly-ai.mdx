...
## Universal Streaming API Feature

AssemblyAI's Universal Streaming API provides enhanced real-time transcription capabilities with improved accuracy and performance. This feature leverages their latest Universal-2 model for superior speech recognition results.

### Enabling Universal Streaming API

To enable the Universal Streaming API in your Vapi assistant configuration, use the `enableUniversalStreamingApi` parameter:
...
- **Enhanced Accuracy**: Utilizes AssemblyAI's latest Universal-2 model for improved transcription quality
- **Better Performance**: Optimized streaming capabilities for real-time applications
- **Reduced Latency**: Faster processing times for live conversations
- **Improved Language Support**: Better handling of diverse accents and languages

### Configuration Options

When using the Universal Streaming API, you can configure additional parameters:
...
- `endpointing`: Controls silence detection threshold (in milliseconds)
- `punctuate`: Enables automatic punctuation insertion
- `formatText`: Applies text formatting for better readability

### Migration from Legacy API

If you're currently using AssemblyAI's standard streaming API, you can easily migrate to the Universal Streaming API by adding the `enableUniversalStreamingApi: true` parameter to your existing configuration. The Universal Streaming API is backward compatible with most existing settings.
